    ...
    CommandLineRunner initDatabase(ItemRepository itemRepository, AuthorRepository authorRepository, UserRepository userRepository, ItemLendingRepository itemLendingRepository, StockRepository stockRepository, CategoryRepository categoryRepository) {
        return args -> {
            if (itemRepository.count() == 0 && authorRepository.count() == 0 && userRepository.count() == 0 && itemLendingRepository.count()==0 && stockRepository.count()==0 && categoryRepository.count()==0) {
                ...
                var user1 = new User("Alim Nurdaulet", "2001/11/11", "Almaty", new Login("nurik@gmail.com", passwordEncoder.encode("123456"),true, Roles.ROLE_USER));
                var user2 = new User("Aitmoldanov Abzal", "2001/27/09", "Almaty", new Login("abzal@gmail.com", passwordEncoder.encode("654789"), true, Roles.ROLE_ADMIN));
                var user3 = new User("Yerkin Adilet", "2000/11/11", "Almaty", new Login("adilet@gmail.com", passwordEncoder.encode("123456"), false, Roles.ROLE_USER));
